...
- **학습**: L1 structure loss + binary domain loss, batch size 128(각 step에서 sim/real 별도 batch), AdamW `lr=1e-3`, cosine schedule, 15 epochs, seed 42. GRL 계수는 Ganin schedule을 쓰며 최대값은 arm별로 고정한다.
- **arm A**: 같은 DANN 코드 경로에서 `lambda_max=0`.
- **arm B**: `lambda_max=1.0`.
- sampler와 augmentation의 RNG를 모델 초기화 RNG와 분리한다. arm A/B의 차이는 `lambda_max`뿐이어야 한다.
- real train은 site 단위 seed 0 split으로 80% domain 학습, 20% 고정 domain probe에 사용한다. probe AUC는 기전 관찰값이며 리더보드 판정을 대체하지 않는다.
...
- **실행 전 중단**: arm 사이의 manifest·seed·학습 step·optimizer 상태가 달라졌거나 test 입력이 domain 학습에 들어가면 실행을 폐기하고 report를 갱신한다.
